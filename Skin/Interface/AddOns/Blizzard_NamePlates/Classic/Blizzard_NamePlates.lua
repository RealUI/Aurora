...
--
-- Blizzard unified the nameplate code across flavors: era and Mists ship the
-- same Blizzard_NamePlate* file set as retail, and only Blizzard_ClassNameplateBar
-- is gated [AllowLoadGameType mainline].  Every symbol this file touches was
-- verified present in all three trees -- NamePlateDriverMixin, OnNamePlateAdded,
-- NamePlateAurasMixin:RefreshAuras / :RefreshLossOfControl, HealthBarsContainer,
-- AurasFrame, DebuffListFrame, CrowdControlListFrame, LossOfControlFrame,
-- ForEachNamePlate, and all three SetClassNameplateBar* driver methods (the
-- methods exist on classic even though the frames never arrive, so the
-- hooksecurefunc calls are safe and stay in for parity).
--
-- This file is therefore a deliberate verbatim copy of Mainline/Blizzard_NamePlates.lua.
-- Keep the two in sync: a plain diff between them should show only this comment.
--------------------------------------------------------------------------------
