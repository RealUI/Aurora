...
function private.AddOns.Blizzard_Transmog()
    ...
    --       OutfitCollection           --
    ----====#####################====----
    local function SkinOutfitCollection()
        local OutfitCollection = TransmogFrame.OutfitCollection
        if OutfitCollection then
            -- Hide decorative backgrounds and gradients
            if OutfitCollection.Background then
                OutfitCollection.Background:SetAlpha(0)
            end
            if OutfitCollection.GradientTop then
                OutfitCollection.GradientTop:SetAlpha(0)
            end
            if OutfitCollection.GradientBottom then
                OutfitCollection.GradientBottom:SetAlpha(0)
            end
            if OutfitCollection.DividerBar then
                OutfitCollection.DividerBar:SetAlpha(0)
            end
            ...
            if OutfitList then
                if OutfitList.DividerTop then
                    OutfitList.DividerTop:SetAlpha(0)
                end
                if OutfitList.DividerBottom then
                    OutfitList.DividerBottom:SetAlpha(0)
                end
                if OutfitList.ScrollBox then
                    Skin.WowScrollBoxList(OutfitList.ScrollBox)
                end
                if OutfitList.ScrollBar then
                    Skin.MinimalScrollBar(OutfitList.ScrollBar)
                end
            end
    ...
    local function SkinCharacterPreview()
        ...
        if CharacterPreview then
            ...
            if CharacterPreview.Gradients then
                if CharacterPreview.Gradients.GradientLeft then
                    CharacterPreview.Gradients.GradientLeft:SetAlpha(0)
                end
                if CharacterPreview.Gradients.GradientRight then
                    CharacterPreview.Gradients.GradientRight:SetAlpha(0)
                end
            end
            ...
            if
                CharacterPreview.ModelScene and CharacterPreview.ModelScene.ControlFrame and
                    Skin.ModelSceneControlFrameTemplateLeftButtonTemplate
             then
                Skin.ModelSceneControlFrameTemplateLeftButtonTemplate(
                    CharacterPreview.ModelScene.ControlFrame.rotateLeftButton
                )
                Skin.ModelSceneControlFrameTemplateRightButtonTemplate(
                    CharacterPreview.ModelScene.ControlFrame.rotateRightButton
                )
            end
    ...
    --      WardrobeCollection          --
    ----====#####################====----
    local function SkinWardrobeCollection()
        local WardrobeCollection = TransmogFrame.WardrobeCollection
        if WardrobeCollection then
            -- Hide the wardrobe background
            if WardrobeCollection.Background then
                WardrobeCollection.Background:SetAlpha(0)
            end
            ...
            if WardrobeCollection.TabHeaders then
                ...
                if WardrobeCollection.TabHeaders.tabPool then
                    _G.hooksecurefunc(
                        WardrobeCollection.TabHeaders.tabPool,
                        "Acquire",
                        function(pool)
            ...
            if TabContent then
                ...
                if TabContent.Background then
                    TabContent.Background:SetAlpha(0)
                end
                if TabContent.Border then
                    TabContent.Border:SetAlpha(0)
                end
                ...
                if ItemsFrame then
                    -- Filter / Search
                    if ItemsFrame.FilterButton then
                        Skin.FilterButton(ItemsFrame.FilterButton)
                    end
                    if ItemsFrame.SearchBox and Skin.SearchBoxNineSliceTemplate then
                        Skin.SearchBoxNineSliceTemplate(ItemsFrame.SearchBox)
                    end

                    -- Weapon dropdown
                    if ItemsFrame.WeaponDropdown then
                        Skin.DropdownButton(ItemsFrame.WeaponDropdown)
                    end

                    -- Divider line (decorative, thin)
                    if ItemsFrame.Divider then
                        ItemsFrame.Divider:SetAlpha(0)
                    end
                ...
                if SetsFrame then
                    if SetsFrame.FilterButton then
                        Skin.FilterButton(SetsFrame.FilterButton)
                    end
                    if SetsFrame.SearchBox and Skin.SearchBoxNineSliceTemplate then
                        Skin.SearchBoxNineSliceTemplate(SetsFrame.SearchBox)
                    end
                end
                ...
                if SituationsFrame then
                    ...
                    if SituationsFrame.SituationFramePool then
                        _G.hooksecurefunc(
                            SituationsFrame.SituationFramePool,
                            "Acquire",
                            function(pool)
